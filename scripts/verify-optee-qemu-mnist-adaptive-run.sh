...
check_log_ordered "artifact_id=mnist-onnx-v1"
...
check_log_ordered "TEAR: profile_id=mnist-default artifact_id=mnist-onnx-v1 backend=onnxruntime-cpu sample=weak7"
check_log_ordered "TEAR: run_id=run-[0-9]+-[0-9]+"
...
check_log_ordered "event=optee_record_decision_ok"
...
check_log_ordered "TEAR: reported_decision run_id=run-[0-9]+-[0-9]+ artifact_id=mnist-onnx-v1 proposal=request_high_accuracy_profile decision=rejected reason=profile_unavailable value=0"
check_log_ordered "event=optimization_decision_reported_by_runtime_manager"
